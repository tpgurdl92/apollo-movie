...
export default ({id, bg, isLiked, small,title,rate,release_date,status}) => {
  const [isMouseOn, setMouseOn] =useState(false);
  const [toggleLike] = useMutation(LIKE_MOVIE,{variables: {id: parseInt(id), isLiked}});
  ...
  return(
        <Container small={small}>
                <ImgContainer>
            <Link to={`/${status}/${id}`}>
                <Poster onMouseOver={()=>setMouseOn(true)} onMouseOut={()=>setMouseOn(false)}  bg={bg} small={small} >
